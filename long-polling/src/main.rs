...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let database_url = std::env::var("DATABASE_URL")
        .map_err(|_| Error::BadConfig("DATABASE_URL env var is missing".to_string()))?; // use |_| for wildcard for map_err closure
    ...
    //
    // By using ".layer(AddExtensionLayer::new(ctx));", ServerContext is injected into all the routes so we can call ChatService's methods.
    // By injecting the ServerContext, we can use the same instance of ChatService across all the routes. (Dependency Injection)
    //
    let app = Router::new()
        .route(
            "/messages",
    ...
    // run server on a background task
    axum::Server::bind(
        &"0.0.0.0:8080"
            .parse()
